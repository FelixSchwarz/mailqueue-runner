...
def _example_message(*, from_=None, to=None, cc=None, bcc=None) -> str:
    base_msg = textwrap.dedent('''
        Subject: Test message

        Mail body
    ''').strip()
    from_line = f'From: {from_}\n' if from_ else ''
    to_line = f'To: {to}\n' if to else ''
    cc_line = f'CC: {cc}\n' if cc else ''
    bcc_line = f'BCC: {bcc}\n' if bcc else ''
    return from_line + to_line + cc_line + bcc_line + base_msg
...
def test_mq_sendmail_with_cronie_parameters(ctx):
    # use from/to headers as generated by cronie
    # "To:" value if no MAILTO is specified in the crontab file
    rfc_msg = _example_message(to='root', from_='"(Cron Daemon)" <root>')
    msg = email.message_from_string(rfc_msg)
    # cronie does not generate a "Message-ID" header
    assert 'Message-ID' not in msg

    # This is the default parameter list used by cronie to email the output of cron jobs.
    cli_params = ['-FCronDaemon', '-i', '-odi', '-oem', '-oi', '-t', '-f', 'root']
    # "-FCronDaemon" has no effect because the message already contains a "From:" header

    # We need to add a custom aliases file so we can map sender ("root") to
    # an actual email address (otherwise pymta would reject the message).
    aliases_path = create_alias_file({'root': 'staff@site.example'}, ctx.tmp_path)
    cli_params = [f'--aliases={aliases_path}'] + cli_params

    _mq_sendmail(cli_params, msg=rfc_msg, ctx=ctx)

    smtp_msg = retrieve_sent_message(ctx.mta)
    assert tuple(smtp_msg.smtp_to) == ('staff@site.example',)
    assert smtp_msg.msg_data == rfc_msg
    # still no "Message-ID" header (but maybe we should add one by default?)
    assert 'Message-ID' not in email.message_from_string(smtp_msg.msg_data)
